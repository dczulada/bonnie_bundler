    ...
    class BundleExporter
      ...
      def calculate
        records.each do |patient|
          # Assigns a new MRN for each export.  This is necessary since we clone patients from year to year
          patient.medical_record_number = UUID.generate
          patient.save!
        end  
      ...
      def export_patients
        ...
        records.each do |patient|
          ...
          # This is the Medicare HIC oid needed for CMS export
          patient.medical_record_assigner = "2.16.840.1.113883.4.572"
          ...
          export_file File.join(path, "json", "#{filename}_#{patient.medical_record_number}.json"), json
        end
      end
      ...
      def export_valuesets
        ...
        # This code will go through each VS and download the VS categories form VSAC
        update_vs_with_categories
      ...
      def update_vs_with_categories
        api = HealthDataStandards::Util::VSApiV2.new('https://vsac.nlm.nih.gov/vsac/ws/Ticket', 'https://vsac.nlm.nih.gov/vsac/svs/RetrieveMultipleValueSets', @config['vsac_info']['vsac_user'], @config['vsac_info']['vsac_password'])
        vsets = HealthDataStandards::SVS::ValueSet.where(bundle_id: @user.bundle_id)
        vsets.each_with_index do |vset, index|
          begin
            vsets_to_update = HealthDataStandards::SVS::ValueSet.where(oid: vset.oid, bundle_id: @user.bundle_id)
            newvsac = api.get_valueset(vset.oid, program: 'MU2', profile: @config['vsac_info']['profile'])
            doc = Nokogiri::XML(newvsac)
            newvset = HealthDataStandards::SVS::ValueSet.load_from_xml(doc)
            vsets_to_update.each do |vset_to_update|
              vset_to_update.categories = newvset.categories
              vset_to_update.concepts = newvset.concepts
              vset_to_update.save
            end
            puts "#{index} of #{vsets.size}"
          rescue
            puts "#{vset.oid} - Where is my value set"
          end
        end
      end
